<div>
  @if(session()->has('message'))
    <div class="alert alert-success text-green-500 text-center bg-green-100 border border-green-400 rounded-lg p-4 mb-4"
      role="alert">
      {{ session()->get('message') }}
    </div>
  @endif
              ...
              <div>
                <div class="inline-flex gap-x-2">
                  ...
                  <a class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg bg-primary border border-primary-line text-primary-foreground hover:bg-primary-hover focus:outline-hidden focus:bg-primary-focus disabled:opacity-50 disabled:pointer-events-none"
                    href="{{ route('admin.fabric.create') }}">
                ...
                @foreach ($this->fabrics as $fkey => $fabric)
                  ...
                  <tr wire:key="fabric-{{ $fabric->id }}">
                    ...
                    <td class="h-px w-72 whitespace-nowrap">
                      <div class="px-6 py-3">
                        <img class="object-cover w-20 h-20" src="{{ asset('storage/' . $fabric->image) }}"
                          alt="{{ $fabric->name }}">
                      </div>
                    </td>
                    ...
                    <td class="size-px whitespace-nowrap">
                      <div class="px-6 py-1.5">
                        <a class="inline-flex items-center gap-x-1 text-sm text-primary decoration-2 hover:underline focus:outline-hidden focus:underline font-medium"
                          href="{{route('admin.fabric.edit', $fabric->id) }}">
                          Edit
                        </a>
                      </div>
                    </td>
                    ...
                    <td class="size-px whitespace-nowrap">
                      <div class="px-6 py-1.5">
                        <a class="inline-flex items-center gap-x-1 text-sm text-red-500 decoration-2 hover:underline focus:outline-hidden focus:underline font-medium"
                          href="#" wire:click="delete({{ $fabric->id }})">
                          Delete
                        </a>
                      </div>
                    </td>
