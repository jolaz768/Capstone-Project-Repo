<div class="min-h-screen bg-layer dark:bg-gray-950">
    ...
    <div class="relative min-h-[420px] md:min-h-[480px] overflow-hidden rounded-b-[40px] shadow-2xl">
        <img src="{{ asset('storage/' . $shop->shop_image) }}" class="absolute inset-0 w-full h-full object-cover"
            alt="{{ $shop->shop_name }}">

        <div class="absolute inset-0 bg-black/60"></div>

        <div class="relative z-10 py-12 md:py-16 flex items-center">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 w-full">
                <div class="max-w-2xl text-white">
                    ...
                    {{-- Description — now can wrap and adjust height --}}
                    <p class="mt-3 md:mt-5 text-base md:text-lg text-slate-200 leading-relaxed">
                        {{ $shop->description }}
                    </p>

                    {{-- Rating --}}
                    <div class="flex items-center gap-3 mt-3 md:mt-5">
                        <div class="text-yellow-400 text-xl md:text-2xl">★★★★★</div>
                        <span class="text-white/80 text-sm md:text-base">120+ Customer Reviews</span>
                    </div>

                    {{-- Operating hours --}}
                    <div class="flex items-center gap-3 mt-3 md:mt-5">
                        <div class="text-green-400 text-xl md:text-sm">Open:</div>
                        <span class="text-white/80 text-sm md:text-base">Business Operating Hours: 8:00 AM - 5:00
                            PM</span>
                    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 mt-10 md:mt-16">
        <div class="flex items-center justify-between mb-6 md:mb-8">
            <div>
                ...
                <p class="text-muted-foreground dark:text-gray-400 mt-1 md:mt-2">Browse tailoring services available in
                    this shop.</p>
            </div>
        </div>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div
                    ...
                    <div class="p-5 md:p-6">
                        <div class="flex items-center justify-between">
                            <h3 class="text-xl md:text-2xl font-bold text-foreground dark:text-gray-100">
                                {{ $service->name }}</h3>
                        </div>
                        ...
                        <div class="mt-4 md:mt-6 flex items-center justify-between">
                            <span
                                class="text-xs md:text-sm text-slate-400 dark:text-gray-500">{{ $service->created_at->diffForHumans() }}</span>
                            <a href="{{ route('booking.create', ['id' => $shop->id]) }}"
            ...
            @empty
                <div class="col-span-full">
                    <div
                        class="bg-white dark:bg-gray-800 rounded-3xl p-8 md:p-10 text-center shadow-lg border dark:border-gray-700">
                        <h3 class="text-xl md:text-2xl font-bold text-slate-700 dark:text-gray-100">No Services
                            Available
                        </h3>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 mt-14 md:mt-20">
        <div class="flex items-center justify-between mb-6 md:mb-8">
            <div>
                ...
                <p class="text-muted-foreground dark:text-gray-400 mt-1 md:mt-2">Explore garments customized by this
                    tailoring shop.</p>
            </div>
        </div>
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            @forelse ($shop->garments as $garment)
                <div
                    ...
                    <div class="relative h-64 md:h-80 overflow-hidden">
                        <img src="{{ $garment->image ? asset('storage/' . $garment->image) : 'https://via.placeholder.com/300x400' }}"
                            class="w-full h-full object-cover" alt="{{ $garment->name }}">
                        <div class="absolute top-3 right-3 md:top-4 md:right-4">
                            <span
                                class="bg-yellow-500 text-black px-3 md:px-4 py-1.5 md:py-2 rounded-full text-xs md:text-sm font-bold shadow-lg">
                    ...
                    <div class="p-5 md:p-6">
                        ...
                        <p class="mt-2 md:mt-3 text-slate-600 dark:text-gray-300 line-clamp-3 text-sm md:text-base">
                            {{ $garment->description }}</p>
                        <h3 class="text-xl md:text-md font-bold text-foreground dark:text-gray-100">Service:
                            {{ $garment->service->name}}</h3>
                        <div class="mt-4 md:mt-6 flex gap-3">
                            {{-- <button
                                class="flex-1 bg-[#0f2342] hover:bg-[#18365f] dark:bg-gray-700 dark:hover:bg-gray-600 text-white py-2 md:py-3 rounded-2xl font-semibold transition text-sm">
                                View
                            </button> --}}
                            <button
            ...
            @empty
                <div class="col-span-full">
                    <div
                        class="bg-white dark:bg-gray-800 rounded-3xl p-8 md:p-10 text-center shadow-lg border dark:border-gray-700">
                        <h3 class="text-xl md:text-2xl font-bold text-foreground dark:text-gray-100">No Garments
                            Available
                        </h3>
        ...
        <div class="bg-[#0f2342] rounded-[40px] overflow-hidden shadow-2xl">
            <div class="grid lg:grid-cols-2 gap-6 md:gap-10 p-6 md:p-10 items-center">
                <div class="text-white">
                    <span
                        class="bg-yellow-500 text-black px-3 md:px-4 py-1.5 md:py-2 rounded-full text-sm font-semibold">
                        Quick Booking
                    </span>
                    <h2 class="text-3xl sm:text-4xl md:text-5xl font-bold mt-4 md:mt-6">Book Your Tailoring Appointment
                    </h2>
                    <p class="mt-3 md:mt-5 text-slate-300 text-base md:text-lg">Schedule your fitting and tailoring
                        services online.</p>
                </div>

                <div class="bg-white dark:bg-gray-800 rounded-3xl p-6 md:p-8 shadow-xl border dark:border-gray-700">
                    <h3 class="text-2xl md:text-3xl font-bold text-black dark:text-gray-100 mb-4 md:mb-6">Create Booking
                    </h3>
                    <form method="GET" action="{{ route('booking.create', ['id' => $shop->id]) }}">
                        <div class="space-y-4 md:space-y-5">
                            <div>
                                <label
                                    class="block mb-1.5 md:mb-2 font-semibold text-slate-700 dark:text-gray-200">Select
                                    Date</label>
